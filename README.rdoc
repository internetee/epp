...
Originally *EPP v1.3.1* by {Ultraspeed}[http://ultraspeed.co.uk]. This fork is maintained by {Estonian Internet (.ee)}[https://internet.ee] (Sergei Tsõganov, +sergei.tsoganov@internet.ee+). Current release: see +VERSION+ or RubyGems.
...
From this gem's directory (the one that contains the +Dockerfile+):
    ...
    $ docker build -t epp-gem .
    $ docker run --rm epp-gem
...
https://github.com/internetee/epp/issues

== Credit

* Original author: Josh Delsman (http://twitter.com/voxxit), {Ultraspeed}[http://ultraspeed.co.uk]
* Current maintainer (this fork): Sergei Tsõganov, {Estonian Internet (.ee)}[https://internet.ee] — sergei.tsoganov@internet.ee
